...
const TopNavbar = ({ navigation }: Props) => {
  ...
  return (
    <nav
      ...
      className="top-navbar border-b border-stone-400 py-2 font-semibold block overflow-x-scroll relative"
    >
      <ul
        role="list"
        className="flex justify-start gap-1 items-center min-w-full text-lg "
      >
        {navigation.map((item) => (
          <li key={item.id} className="group relative">
            ...
            {item.items && (
              <ul
                role="list"
                className="absolute space-y-1 hidden xl:group-hover:block py-1 border border-stone-200 rounded-md bg-white dark:bg-secondary z-20 shadow-lg"
              >
                {item.items.map((subItem) => (
                  <li key={subItem.id}>
                    <Link
                      href={subItem.href}
                      aria-label={`View more ${subItem.title} articles`}
